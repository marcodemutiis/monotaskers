...
void LedPinOn() {
  ...
  for (int i = 0; i < btns; i++) {
    ...
    {
      //      noteOff(drummachineInstr[i],0,1);
      noteOff(channel, drummachineInstr[i], 0);
    }
  }
  ...
  for (int i = 0; i < btns; i++) {
    if (drummachineSteps[ledPinCount][i] == true)
    {
      ...
      //      noteOn(drummachineInstr[i],127,1);
      noteOn(channel, drummachineInstr[i], 127);
...
void clickEvent(int i) {
  ...
  if (drummachineSteps[ledPinCount][i]) noteOn(channel, drummachineInstr[i], 127);
  else noteOff(channel, drummachineInstr[i], 0);
